...
def finding_mode_from_geometry(geometry, frequency=1, resolution=20, time=50):
    """
    This function generates the Ez and Hy field cross sections 
    for the fundamental mode of the geometry in input.
    ...
    x_max_index = int(pml_thickness * resolution) + 3
    ...
    Ez1_cross = np.abs(Ez1_cross) / np.sqrt(Power_value)
    Hy1_cross = np.abs(Hy1_cross) / np.sqrt(Power_value)

    # Return the field cross section 
    return Ez1_cross, Hy1_cross
