...
if not width_found:
        ...
        wvg_widths = [0.4, 0.35, 0.3, 0.25, 0.2] 
        ...
        for frq_idx, frq in enumerate(frq_simulated):
                ...
                for wvg_width in wvg_widths:
                        ...
                        # Mode found
                        Ez, Hy, neff = SL.finding_mode_from_geometry(geometry, 
                                                                       frequency=frq,
                                                                       mode=2,
                                                                       resolution=sim_resolution)
                
                        # Define axis of the mode
                        cross_axis = np.linspace(-sim_width/2, sim_width/2, len(Ez))
                
                        # Plotting with different colors/styles for each width
                        ax.plot(cross_axis, Ez, 
                ...
                ax.legend(fontsize=plt_fontsize-2, loc='best')
...
if not mode_calculated:
        ...
        wvg_width = 0.25 if mode_calculated_kind == 'wvg' else 4.5
...
if not propagation_waveguide:
        ...
        for frq in frq_simulated:
                ...
                source = SL.generate_modal_source(Ez_cross = Ez,
                                                  Hy_cross = Hy,
                                                  cross_axis = np.linspace(-sim_width/2, sim_width/2, len(Ez)),
        ...
        for idx, frq in enumerate(frq_simulated):
                ...
                # Calculate overlap
                overlap, _ = SL.calculate_modal_overlap(Ez_cross = Ez_array[idx], 
                                                        Hy_cross = Hy_array[idx], 
                                                        Ez_field = ez_data, 
                                                        Hy_field = hy_data,
...
if not propagation_curve:
        ...
        sim_resolution = 25
        sim_sub_pixel = 10
        sim_circle_count = 200
        ext_width = 3
        wvg_width = 0.25
        wvg_neff = 2
        cld_neff = 1.3
        sim_width = ext_width * 2 
        sim_length = ext_width * 2

        # Define different waveguide radii to test
        wvg_radii = [30, 13, 11]  # um
        ...
        for wvg_radius in wvg_radii:
                ...
                for idp in range(sim_circle_count + 1):
                        angle = idp / sim_circle_count * 2 * np.pi
                ...
                center=mp.Vector3(-wvg_radius-1/sim_resolution),
                size=mp.Vector3(ext_width * 2, cell_size.y))
                ...
                eps_cross = eps_data[:len(Ez_profile), y_center_index]
  
                # Spatial discretization step
                delta = dft_volume.size.x / sim_resolution
                ...
                Ez_normalize, Hx_normalize, neff = SL.normalizing_mode_field(
                        ...
                        delta=delta, eps_cross=eps_cross)
                
                # Plot normalized field profile
                cross_axis = np.linspace(ext_width, -ext_width, len(Ez_normalize))
                ax1.plot(cross_axis, Ez_normalize, label=f'r={wvg_radius}μm, neff={neff:.3f}', linewidth=2)

                ### SECOND METHOD ###
                # Create the material 
                Nx = int(sim_length * sim_resolution * sim_sub_pixel)
                Ny = int(sim_width * sim_resolution * sim_sub_pixel)
                y_shift = max(sim_width / 2 + wvg_radius * (1 / cld_neff - 1), 0)

                #Vectorized Coordinate Generation
                j_indices = np.arange(Ny)

                # Convert indices to simulation coordinates
                y_coords = (j_indices - Ny/2) / (sim_resolution * sim_sub_pixel) + y_shift

                # Create Base mask for Index Profile
                core_mask = np.abs(y_coords) <= wvg_width/2
                n_profile_high_res = np.where(core_mask, wvg_neff, cld_neff)

                # Generate Weights (With Bend Factor)
                weights_1d = (n_profile_high_res * (1 + y_coords/wvg_radius))**2

                # Broadcast to 3D for Meep (Nx, Ny, 1)
                weights_2d = np.tile(weights_1d, (Nx, 1))
                max_weight = np.max(weights_2d)
                weights3d = weights_2d[:, :, None] / max_weight

                # Create the material grid
                material_grid = mp.MaterialGrid(
                mp.Vector3(Nx, Ny, 1),
                medium1=mp.Medium(epsilon=1e-6),
                ...
                # Generate eps_profile for Mode Solver
                eps_high_res = n_profile_high_res**2

                # Downsample from 'sub_pixel' resolution to standard 'sim_resolution' (robust)
                target_len = int(np.round(sim_width * sim_resolution))
                limit = target_len * sim_sub_pixel

                arr = np.asarray(eps_high_res).flatten()
                if arr.size < limit:
                        arr = np.pad(arr, (0, limit - arr.size), mode='edge')
                elif arr.size > limit:
                        arr = arr[:limit]

                if target_len <= 0:
                        eps_cross = np.array([])
                elif sim_sub_pixel > 1:
                        eps_cross = arr.reshape(target_len, sim_sub_pixel).mean(axis=1)
                else:
                        eps_cross = arr[:target_len]

                # Find the Mode based on the geometry
                Ez1_cross, Hy1_cross, neff = SL.finding_mode_from_geometry(
                        geometry, frequency=frq, mode=1, 
                        resolution=sim_resolution, eps_cross=eps_cross)
                ...
                ax2.plot(cross_axis, Ez1_cross, 
                        label=f'r={wvg_radius}μm, neff={neff:.3f}', linewidth=2)

        # Configure axes
        plt_fontsize = 16
        ax1.set_title('Ez Mode Profile - Ring', fontsize=plt_fontsize, fontweight='bold')
        ax1.set_xlabel('Position [μm]', fontsize=plt_fontsize)
        ...
        # Configure subplot with larger fonts
        ax2.set_title('Ez Mode Profile - Tilt Neff', fontsize=plt_fontsize, fontweight='bold')
        ax2.set_xlabel('Position [μm]', fontsize=plt_fontsize)
        ax2.set_ylabel('Normalized Field', fontsize=plt_fontsize)
        ax2.tick_params(axis='both', which='major', labelsize=plt_fontsize)
        ax2.legend(fontsize=plt_fontsize-2, loc='best')
        ax2.grid(True)
